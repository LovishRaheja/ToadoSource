...
public class ChatHelper {
    ...
    public static ChatMessageRealm maingson;
...
//    }
    ...
    public static void addChatMsgRealm(final Message message, final String mykey, final Activity context) {
        ...
        re.executeTransactionAsync(new Realm.Transaction() {
            ...
            public void onError(Throwable error) {
                // Transaction failed and was automatically canceled.
                System.out.println(message.getBody().contains("\"msgstatus\":\"3\"") + "  new message stored realm failed chat helper    " + message.getStanzaId() + " " + message.getBody());
    ...
    public static void addMessageRead(final Context contx, final String msgid) {
        try {
            ...
            Log.d(TAG, result2.get(1).getMsgstatus() + "sending broadcast for msg read " + result2.get(1).getMsgstring());

            if (!result2.get(1).getMsgstatus().matches("3")) {
                re.beginTransaction();
                ChatMessageRealm cm = new ChatMessageRealm(result2.get(1).getChatref(), result2.get(1).getOtherjid(), result2.get(1).getMsgstring(), result2.get(1).getSenderjid(), result2.get(1).getSendertime(), result2.get(1).getSenderdate(), result2.get(1).getMsgtype(), result2.get(1).getMsgid(), "3", result2.get(1).getMsgweburl(), result2.get(1).getMsglocalurl(), result2.get(1).getMediathumbnail());
                re.copyToRealmOrUpdate(cm);
